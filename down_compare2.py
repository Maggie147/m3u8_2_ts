...
@func_timer
def download_use_process(url_list, save_path):
    """
    使用多进程实现
    """
    que = Queue()
    process_list = []
    for url in url_list:
        p = TsDownload(url, que, save_path)   
        p.start() 
...
def main():
    ...
    ts_path = './ts_download'
    download_use_process(urls, ts_path)
